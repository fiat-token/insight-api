...
function _createrawtransaction(senders, recipients) {
  return lib.rpcCall("createrawtransaction", [senders, recipients])
};

function _signrawtransaction(hex, privKey) {
  return lib.rpcCall("signrawtransaction", [hex, null, [privKey]])
};

function _sendrawtransaction(hex) {
  return lib.rpcCall("sendrawtransaction", [hex])
};

module.exports.broadcastTransaction = function (req, res) {
  var txidToSpend = req.body.txidToSpend;
  var vout = req.body.vout * 1;
  var amount = req.body.amount / 10000;
  var address = req.body.address;
  var privKey = req.body.privKey;
  var opreturn = req.body.opreturn;

  try {
    var senders = [{ "txid": txidToSpend, "vout": vout }];
    var recipients = { "data": opreturn, [address]: amount };

    _createrawtransaction(senders, recipients)
      // Create raw Transaction result
      .then(function (hexToSign) {
        if (hexToSign.error) {
          console.log("createrawtransaction: err:" + JSON.stringify(hexToSign.error));
          lib.sendRes(res, 500, hexToSend.error);
        }

        return _signrawtransaction(hexToSign.result, privKey);
      })
      // Sign Raw Transaction result
      .then(function (hexToSend) {
        if (hexToSend.error) {
          console.log("signrawtransaction: err:" + JSON.stringify(hexToSend.error));
          lib.sendRes(res, 500, hexToSend.error);
        }

        return _sendrawtransaction(hexToSend.result.hex);
      })
      // Send Raw Transaction result
      .then(function (newtx) {
        if (newtx.error) {
          console.log("sendrawtransaction: err:" + JSON.stringify(newtx.error));
          lib.sendRes(res, 500, newtx.error);
        }

        lib.sendRes(res, 200, { 'txid': newtx.result });
      })
      .catch(function (err) {
        console.log("broadcastTransaction chain error");
        console.log(err);
        lib.sendRes(res, 500, err);
      })
  }
  catch (err) {
    console.log("broadcastTransaction: err:" + JSON.stringify(err));
    lib.sendRes(res, 500, err);
  }
